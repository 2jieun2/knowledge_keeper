...
class PyramidPoolingModule(nn.Module):
    def __init__(self, in_c):
        ...
        self.conv1x1_1 = nn.Sequential(
            nn.Conv3d(in_c, in_c//4, kernel_size=1, stride=1, padding=0, bias=False)
        )
    ...
    def forward(self, x):
        ...
        p1 = self.spatial_pyramid_pool(x, levels[0])
        p2 = self.spatial_pyramid_pool(x, levels[1])
        p3 = self.spatial_pyramid_pool(x, levels[2])
        p4 = self.spatial_pyramid_pool(x, levels[3])

        p1 = self.conv1x1_1(p1)
        p2 = self.conv1x1_2(p2)
        p3 = self.conv1x1_3(p3)
        p4 = self.conv1x1_4(p4)

        p1 = F.upsample(p1, size=x.shape[2:], mode='trilinear')
        p2 = F.upsample(p2, size=x.shape[2:], mode='trilinear')
        p3 = F.upsample(p3, size=x.shape[2:], mode='trilinear')
        p4 = F.upsample(p4, size=x.shape[2:], mode='trilinear')
    ...
    def spatial_pyramid_pool(self, x, level, mode='max'):
        b, c, h, w, d = x.shape

        h_kernel = int(math.ceil(h / level))
        w_kernel = int(math.ceil(w / level))
        d_kernel = int(math.ceil(d / level))
        ...
        d_pad1 = int(math.floor((d_kernel * level - d) / 2))
        d_pad2 = int(math.ceil((d_kernel * level - d) / 2))

        padded = F.pad(input=x, pad=[w_pad1, w_pad2, h_pad1, h_pad2, d_pad1, d_pad2], mode='constant', value=0)
        if mode == "max":
            pool = nn.MaxPool3d((h_kernel, w_kernel, d_kernel), stride=(h_kernel, w_kernel, d_kernel), padding=(0, 0, 0))
        elif mode == "avg":
            pool = nn.AvgPool3d((h_kernel, w_kernel, d_kernel), stride=(h_kernel, w_kernel, d_kernel), padding=(0, 0, 0))
        out = pool(padded)
...
class SegEncoder(nn.Module):
    def __init__(self, args, in_c=1):
        ...
        self.conv1 = RecursiveResidualBlock(in_c, nf)
        self.conv2 = RecursiveResidualBlock(nf, nf*2)
        self.conv3 = RecursiveResidualBlock(nf*2, nf*4)
        self.conv4 = RecursiveResidualBlock(nf*4, nf*8)
...
class SegDecoder(nn.Module):
    def __init__(self, args, out_c=4):
        ...
        self.up1 = RRUpMerge(nf*16, nf*8, kernel=k, pad=p)
        self.up2 = RRUpMerge(nf*8, nf*4, kernel=k, pad=p)
        self.up3 = RRUpMerge(nf*4, nf*2, kernel=k, pad=p)
        self.up4 = RRUpMerge(nf*2, nf, kernel=k, pad=p)

        self.pp = PyramidPoolingModule(nf)

        self.out = nn.Sequential(
            nn.Conv3d(nf*2, out_c, kernel_size=1, stride=1, bias=False)
        )

    def forward(self, enc_list, return_all=False):
...
class Classifier(nn.Module):
    def __init__(self, args, out_c=4):
        ...
        self.conv1x1_1 = nn.Sequential(
            nn.Conv3d(nf*8, out_c, kernel_size=1, stride=1, padding=0, bias=False)
        )
        self.conv1x1_2 = nn.Sequential(
            nn.Conv3d(nf*4, out_c, kernel_size=1, stride=1, padding=0, bias=False)
        )
        self.conv1x1_3 = nn.Sequential(
            nn.Conv3d(nf*2, out_c, kernel_size=1, stride=1, padding=0, bias=False)
        )
        self.conv1x1_4 = nn.Sequential(
            nn.Conv3d(nf, out_c, kernel_size=1, stride=1, padding=0, bias=False)
        )

    def forward(self, u1, u2, u3, u4):
        to_size = u4.shape[2:]

        u1 = F.upsample(u1, size=to_size, mode='trilinear')
        u2 = F.upsample(u2, size=to_size, mode='trilinear')
        u3 = F.upsample(u3, size=to_size, mode='trilinear')

        u1 = self.conv1x1_1(u1)
        u2 = self.conv1x1_2(u2)
        u3 = self.conv1x1_3(u3)
...
class Implementation(object):
    ...
    def training(self, device, fold, datetime_train):
        ...
        classifier = nn.DataParallel(Classifier(self.args)).to(device)
        classifier.apply(weights_init_normal)
        optimizer_C = torch.optim.Adam(classifier.parameters(), lr=self.lr, betas=(0.9, 0.999))
            ...
            for i, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc='Batch'):
                ...
                if self.guided:
                    if self.train_net == 'ED':
                        ...
                        u1, u2, u3, u4, pred_y = decoder(enc_list, return_all=True)
                    else:
                        u1, u2, u3, u4, pred_y = decoder(keeper(real_x), return_all=True)
                else:
                    u1, u2, u3, u4, pred_y = decoder(encoder(real_x), return_all=True)

                u1, u2, u3, u4 = classifier(u1, u2, u3, u4)

                loss_total = loss_CCE(u1, real_y) + loss_CCE(u2, real_y) + loss_CCE(u3, real_y) + loss_CCE(u4, real_y) + loss_CCE(pred_y, real_y)
