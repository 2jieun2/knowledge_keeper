...
class VoxResModule(nn.Module):
    def __init__(self, in_c):
        ...
        self.model = nn.Sequential(
            ...
            nn.Conv3d(in_c, in_c, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
            nn.BatchNorm3d(in_c),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_c, in_c, kernel_size=(3, 3, 3), padding=1),
        )
...
class SegEncoder(nn.Module):
    def __init__(self, args, in_c=1):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(in_c, nf, kernel_size=(3, 3, 3), padding=1),
            nn.BatchNorm3d(nf),
            nn.ReLU(inplace=True),
        ...
        self.conv2 = nn.Sequential(
            nn.BatchNorm3d(nf),
            nn.ReLU(inplace=True),
            nn.Conv3d(nf, nf*2, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1)
        )
        self.vrm2 = VoxResModule(nf*2)
        self.vrm3 = VoxResModule(nf*2)

        self.conv3 = nn.Sequential(
            nn.BatchNorm3d(nf*2),
            nn.ReLU(inplace=True),
            nn.Conv3d(nf*2, nf*4, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1)
        )
        self.vrm5 = VoxResModule(nf*4)
        self.vrm6 = VoxResModule(nf*4)

        self.conv4 = nn.Sequential(
            nn.BatchNorm3d(nf*4),
            nn.ReLU(inplace=True),
            nn.Conv3d(nf*4, nf*8, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1)
        )
        self.vrm8 = VoxResModule(nf*8)
        self.vrm9 = VoxResModule(nf*8)

        self.conv5 = nn.Sequential(
            nn.BatchNorm3d(nf*8),
            nn.ReLU(inplace=True),
            nn.Conv3d(nf*8, nf*16, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=1)
        )
        self.vrm11 = VoxResModule(nf*16)
        self.vrm12 = VoxResModule(nf*16)
...
class SegDecoder(nn.Module):
    def __init__(self, args, out_c=4):
        ...
        self.deconv5 = nn.Sequential(
            nn.ConvTranspose3d(nf*16, nf*16, kernel_size=(16, 16, 16), stride=(16, 16, 16)),
            nn.Conv3d(nf*16, out_c, kernel_size=1)
        )
        self.deconv4 = nn.Sequential(
            nn.ConvTranspose3d(nf*8, nf*8, kernel_size=(8, 8, 8), stride=(8, 8, 8)),
            nn.Conv3d(nf*8, out_c, kernel_size=1)
        )
        self.deconv3 = nn.Sequential(
            nn.ConvTranspose3d(nf*4, nf*4, kernel_size=(4, 4, 4), stride=(4, 4, 4)),
            nn.Conv3d(nf*4, out_c, kernel_size=1)
        )
        self.deconv2 = nn.Sequential(
            nn.ConvTranspose3d(nf*2, nf*2, kernel_size=(2, 2, 2), stride=(2, 2, 2)),
            nn.Conv3d(nf*2, out_c, kernel_size=1)
        )
        self.deconv1 = nn.Sequential(
            nn.ConvTranspose3d(nf, nf, kernel_size=(1, 1, 1), stride=(1, 1, 1)),
            nn.Conv3d(nf, out_c, kernel_size=1)
        )
    ...
    def forward(self, enc_list, return_all=False):
        ...
        to_size = c1.shape[2:]
        c2 = match_size(c2, to_size)
        c3 = match_size(c3, to_size)
        c4 = match_size(c4, to_size)
        c5 = match_size(c5, to_size)
        out = c1+c2+c3+c4+c5
            ...
            for i, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc='Batch'):
                ...
                if self.guided:
                    if self.train_net == 'ED':
                        ...
                        g1 = match_size(g1, e1.shape[2:])
                        g2 = match_size(g2, e2.shape[2:])
                        g3 = match_size(g3, e3.shape[2:])
                        g4 = match_size(g4, e4.shape[2:])
                        g5 = match_size(g5, e5.shape[2:])
                        enc_list = fusion([g1, g2, g3, g4, g5], [e1, e2, e3, e4, e5])
                        u1, u2, u3, u4, u5, pred_y = decoder(enc_list, return_all=True)
                    else:
                        u1, u2, u3, u4, u5, pred_y = decoder(keeper(real_x), return_all=True)
                else:
                    u1, u2, u3, u4, u5, pred_y = decoder(encoder(real_x), return_all=True)

                loss_main = loss_CCE(pred_y, real_y)
                loss_aux = loss_CCE(u1, real_y) + loss_CCE(u2, real_y) + loss_CCE(u3, real_y) + loss_CCE(u4, real_y)
                loss_aux *= w_aux
